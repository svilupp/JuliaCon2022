...
inputs = build_inputs(df;
                      col_target = "revenue", col_datetime = "dt",
                      col_time_std = "time_std", col_cat = "events",
                      cols_context = [c
                                      for c in col_names.cols_context if c != "newsletters"],
                      cols_organic = ["newsletters"],
                      cols_hols = ["hols_ind"],
                      cols_spend = col_names.cols_spend,
                      seasonality_periods = [4.0],
                      spline_degree = 0);
#----------------------------------------------------------------------------
...
p1 = set_priors(ParamsStage1(model_name = Val(Symbol(MODEL_NAME_PREFIT)),
                             scales_trend = 0.2,
                             scales_hols = 0.3,
                             scales_noise = 0.2,
                             cat_levels = 1),
...
img = plot_model_fit_by_period(inputs.y_std,
                               sum(stage1fit.stage1_fitted_trends, dims = 2) |> vec,
...
p2 = set_priors(ParamsStage2(model_name = Val(Symbol(MODEL_NAME)),
