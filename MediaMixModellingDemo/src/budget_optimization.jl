...
function convert_budget_multiplier_to_spend_multiplier(spend_prev_trf,factor_to_scale_spend_to_orig,budget_multiplier)
...
function simulate_revenues_summed(chain_optim,model_optim,optim_mask)
    ...
    simulations_summed=hcat([s.y for s in simulations]...) |> x->@view(x[optim_mask,:])|>sum_columns|>vec
...
function workflow_budget_to_simulation(chain_optim,model_orig,
                    X_spend,optim_mask,
                    spend_prev_trf,factor_to_scale_spend_to_orig,budget_multiplier)
    
    # assumes X_spend is already a Matrix
    spend_mutliplier=convert_budget_multiplier_to_spend_multiplier(
                spend_prev_trf,factor_to_scale_spend_to_orig,budget_multiplier)
    ...
    simulations_new=simulate_revenues_summed(chain_optim,model_stage2a(model_args_new...),optim_mask)
...
function generate_objective_func(chain_optim,model_orig,
                    X_spend,optim_mask,
                    spend_prev_trf,factor_to_scale_spend_to_orig,loss_func=identity,simulations_basecase=nothing,bounds=nothing)
    # assumes X_spend is already a Matrix
    ...
    return budget_multiplier-> begin
        ...
        simulations_new,X_spend_new,spend_mutliplier_new=workflow_budget_to_simulation(
...
function objective_func_parallel(budget_multipliers)
    ...
    Threads.@threads for i in axes(budget_multipliers,1)
        @inbounds f_,g_,h_=objective_func2(@view budget_multipliers[i,:])
        @inbounds fx[i]=f_
        @inbounds gx[i,:] .= g_
        # @inbounds hx[i]=h_[1]
    end
