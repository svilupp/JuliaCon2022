function create_time_index(start_date::Union{Date,String},length::Integer)
    ...
    df=@chain begin
        DataFrame(:dt=>Date(start_date):Day(cycle_length):(Date(start_date)+Day(cycle_length*length-1)))
...
function create_marketing_spend(time_idx,configs,coefs)
    ...
    function marketing_spend_factory(time_idx,label,spend_mean,spend_std,spend_sparsity,unit_scale_ratio)
        len=length(time_idx)
        # center at spend_mean
        input=spend_mean .+ randn(len) .* spend_std 
        # bound to 0-1 interval
        input=min.(max.(input,0),1);

        # apply desired sparsity (1. = no sparsity, 0. = all zeros)
        sparsity_mask=randsubseq(1:len,1-spend_sparsity)
    ...
    function marketing_effect_factory(input,decay_rate,halfpoint,slope,coef,unit_scale_ratio)
...
function create_dataset(start_date,len,seed=nothing)
    ...
    marketing_configs=[(label="facebook_S",mean=0.4,std=0.2,sparsity=1.,
                        decay_rate=0.5,halfpoint=0.4,slope=1.5,
                        unit_scale_ratio=0.05),
                        (label="tv_S",mean=0.8,std=0.2,sparsity=.7,
                        decay_rate=0.8,halfpoint=0.8,slope=0.9,
                        unit_scale_ratio=0.1),
                        (label="search_S",mean=0.5,std=0.3,sparsity=1.,
                        decay_rate=0.05,halfpoint=0.5,slope=2.,
                        unit_scale_ratio=0.05),
                        ]
    coefs=[2.,1.3,2.5]
    ...
    @info let marketing_configs=marketing_configs,coefs=coefs
        marketing_msg=[merge(cfg,(;coef)) for (cfg,coef) in zip(marketing_configs,coefs)]|>
                x->join(x," |\n ")
        roas_msg=[@sprintf("%s: %.1f",cfg.label,coef) for (cfg,coef) in zip(marketing_configs,coefs)]|>
                x->join(x," | ")
    ...
    trend_seasonality_holidays_noise=5 .* sum([y_contribs[begin:3]...,randn(len) .* noise_scale])
    ...
    cols_y=["trend","seas","hols","events",names(X_spend)...,names(X_context)...,"noise"]
