...
# example: roas_total=calc_roas_total(stage2_fit_allsamples,X_spend_raw,revert_pipe_y)
# Expects all variables to be masked already (subset)
function calc_roas_total(generated_quantities, X_spend_raw, revert_y_func)
    # extract mean effect by each variable
    adspend_effect_mean = [tup.mu_spend_by_var for tup in generated_quantities] |> mean |>
                          vec

    effect_raw = sum(revert_y_func(adspend_effect_mean))
