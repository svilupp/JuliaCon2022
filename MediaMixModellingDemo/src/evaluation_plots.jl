...
function plot_model_fit_by_period(y_true,y_pred,p)
    ...
    annotation_points=[(0,plt_max),(0,plt_max*.95),(0,plt_max*.9)]
    ...
    annotate!(pl,annotation_points[1]...,text("Goodness of Fit","helvetica bold",10,:black,:top,:left))
    annotate!(pl,annotation_points[2]...,text(@sprintf("Pseudo R2: %.1f%%",pseudor2(y_true,y_pred)*100),"helvetica",8,:black,:top,:left))
    annotate!(pl,annotation_points[3]...,text(@sprintf("NRMSE: %.1f%%",nrmse(y_true,y_pred)*100),"helvetica",8,:black,:top,:left))
...
function plot_contributions(effect_shares,cols,p)
    ...
    temp_to_plot=@chain begin 
        DataFrame(:labels=>categorical(cols,levels=cols),
            ...
            :effect_share=>effect_shares|>percentage_share|>vec|>x->max.(x,1e-3)
        )
        reverse
    ...
    pl=@df temp_to_plot groupedbar(levelcode.(:labels),:value,
            ...
            legend=false,
            title="Revenue Contributions",titlefontsize=p.title_fontsize)

    # Labels
    for row in eachrow(unstack(temp_to_plot,:variable,:value;fill=0))
        annotate!(pl,(row.base+row.effect_share)+.01,levelcode(row.labels),
...
function plot_effects_vs_spend(effect_shares,spend_shares,cols,p)
    ...
    temp_to_plot=@chain begin 
        DataFrame(:labels=>cols,:effect_share=>effect_shares,:spend_share=>spend_shares)
        DataFramesMeta.stack(_,[:effect_share,:spend_share])
        @transform :variable=categorical(prettify_labels.(:variable)) :labels=categorical(:labels;levels=cols)
    end
    ...
    pl=@df temp_to_plot groupedbar(:labels,:value,
            ...
            color=plt_colors,
            xlim=plt_xlim,
            title="Share of Spend vs Share of Effect",titlefontsize=p.title_fontsize)


    for row in eachrow(temp_to_plot)
        y_offset= row.variable==plt_groups[1] ? -.7 : -.3
        annotate!(pl,row.value+.01,levelcode(row.labels)+y_offset,text(pct_formatter1f(row.value),8,:black,:mid,:left))
    end
...
function plot_response_curves_table(decay_rates,roass,mroas_at_means,cols,roas_total,p)
    ...
    # TOTAL ROAS
    annotate!(pl,0.5,0.1,
        text(@sprintf("Overall ROAS: %.1fx",roas_total),p.table_fontsize_header,:black,:mid,:center))
    
    # Headers
    annotate!(pl,0.2,length(cols)-0.15,text("Decay Rate",p.table_fontsize_header,:black,:mid,:center))
    annotate!(pl,0.5,length(cols)-0.15,text("ROAS",p.table_fontsize_header,:black,:mid,:center))
    annotate!(pl,0.8,length(cols)-0.15,text("mROAS (@mean)",p.table_fontsize_header,:black,:mid,:center))
    
    for (label,decay_,roas_,mroas_) in zip(plt_labels,decay_rates,roass,mroas_at_means)
        annotate!(pl,0.2,levelcode(label)-0.5,text(pct_formatter1f(decay_),p.table_fontsize_body,:black,:mid,:center))
        annotate!(pl,0.5,levelcode(label)-0.5,text(@sprintf("%.1fx",roas_),p.table_fontsize_body,:black,:mid,:center))
        annotate!(pl,0.8,levelcode(label)-0.5,text(@sprintf("%.1fx",mroas_),p.table_fontsize_body,:black,:mid,:center))
    end
...
function plot_mmm_one_pager(plot_array,frame_idx::Int64,p)
    ...
    pl_empty=plot(title = "", grid = false, showaxis = false, ticks=false,bottom_margin = -0Plots.px)
...
function plot_optimized_spend_share_comparison(spend_share_prev,spend_share_optim,cols,p)
    ...
    # Create utility dataframe
    temp_to_plot=@chain begin 
        DataFrame(:labels=>cols,:previous=>spend_share_prev,:optimized=>spend_share_optim)
        DataFramesMeta.stack(_,[:optimized,:previous])
        @transform :variable=categorical(prettify_labels.(:variable),levels=prettify_labels.(["Optimized","Previous"])) :labels=categorical(:labels;levels=cols)
    end
    ...
    pl=@df temp_to_plot groupedbar(:labels,:value,
            ...
            alpha=plt_alphas,
            xlim=plt_xlim,
            title="Proposed Mkt. Budget Allocation",titlefontsize=p.title_fontsize)
...
function plot_optimized_contribution(effect_prev,effect_optim,roas_total,optim_start,optim_end,p)

    # utility function
    extract_effect_raw=x->revert_pipe_y([x]).y[1]
    ...
    annotate!(pl,0.2,pl_max*1.15,text("Period: $(optim_start) - $(optim_end)",
            "helvetica bold",pplot.table_fontsize_body,:black,:mid,:left))
    annotate!(pl,0.2,pl_max*1.1,text("ROAS: "*@sprintf("%.1fx",roas_total),"helvetica bold",p.table_fontsize_body,:black,:mid,:left))
    annotate!(pl,0.2,pl_max*1.05,text("Uplift (AVG): "*@sprintf("%.1f",uplift_delta)*" / "*pct_formatter1f(uplift_perc),
            "helvetica bold",pplot.table_fontsize_body,:black,:mid,:left))

    # Plot labels
    let prev_=extract_effect_raw(effect_prev),optim_=extract_effect_raw(effect_optim)
        annotate!(pl,0.5,prev_*1.05,text(float_formatter1f(prev_),p.table_fontsize_body,:black,:bottom,:center))
        annotate!(pl,1.5,optim_*1.05,text(float_formatter1f(optim_),p.table_fontsize_body,:black,:bottom,:center))
    end
...
function plot_optimized_uplift_histogram(revenue_uplift,p)
    ...
    annotate!(pl,ymin,pl_max*1.15,text("Uplift AVG: "*float_formatter1f(uplift_avg)*" / StDev: "*float_formatter1f(uplift_std),
            "helvetica bold",p.table_fontsize_body,:black,:mid,:left))
    annotate!(pl,ymin,pl_max*1.1,text("Percentage of profitable scenarios: "*pct_formatter1f(mean(revenue_uplift.>0)),
            "helvetica bold",p.table_fontsize_body,:black,:mid,:left))
