...
# TO DO: Define a constructor based on these variables!
# TO DO: Create a function to show implications of selected priors (eg, distribution/values and implications for output values when used with X)

# TO DO: Re-write to be agnostic of whether datasets are or aren't provided (ie, accept `nothing`)
...
@model function model_stage2a(time_index, X_trend, X_spend, X_org, X_context, p,
    ...
    for j in axes(X_spend_transformed, 2)
        @simd for i in axes(X_spend_transformed, 1)
            @inbounds X_spend_transformed[i, j] = hill_curve(eps_t +
                                                             X_spend_transformed[i, j],
...
function quick_nuts_diagnostics(chain, max_depth)
